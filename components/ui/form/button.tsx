...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ 
    ...
    // Handle children based on asChild
    let childContent = children;
    
    // Only manipulate children if not using asChild
    if (!asChild) {
      let childrenArray = React.Children.toArray(children);
      
      // If there's an icon (SVG element) and other content
      ...
      // Loading spinner should always come first
      if (isLoading) {
        childrenArray = [
          <span 
            key="loading-spinner"
